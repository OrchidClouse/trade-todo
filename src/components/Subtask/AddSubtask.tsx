      ...
      {state.isCreateSubtaskFormOpen ? (
        <form className={styles.addTaskForm}>
          ...
          <div className={styles.formButtons}>
            <Button size='small' type='primary' title='Add' onClick={(e) => handleForm(e, true)} disabled={!state.title} />
            <Button size='small' type='delete' title='Cancel' onClick={(e) => handleForm(e, false)} />
          </div>
        </form>
      ) : (
        <Button size='small' title='New subtask' onClick={() => dispatchState({ type: 'OPEN_FORM' })} />
      )}
    </>
  );
};